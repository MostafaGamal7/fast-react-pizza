...
function Button({ children, disabled, to, type, onClick }) {
  ...
  const styles = {
    ...
    round: base + "px-3 py-1 text-sm md:px-4 md:py-2",
    secondary:
      "px-4 py-2.5 text-sm md:px-6 md:py-3.5 rounded-full border-2 border-stone-300 font-semibold uppercase tracking-wide text-stone-400 hover:text-stone-800 transition-colors duration-300 hover:bg-stone-300 focus:bg-stone-300 focus:text-stone-800 focus:outline-none focus:ring focus:ring-stone-200 focus:ring-offset-2 disabled:cursor-not-allowed ",
  };
  if (to)
    return (
